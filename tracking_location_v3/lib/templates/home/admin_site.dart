...
class Admin_site extends StatefulWidget {
  const Admin_site({Key? key, required this.uuid, required this.name}) : super(key: key);
  final uuid;
  final name;
  @override
  _Admin_siteState createState() => _Admin_siteState(uuid: uuid);
}

class _Admin_siteState extends State<Admin_site> {
  _Admin_siteState({Key? key, required this.uuid});
  final uuid;
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: RaisedButton(
                ...
                shape: new RoundedRectangleBorder(
                    borderRadius: new BorderRadius.circular(30.0)),
                child: Text("Quản lý truy vết"),
                onPressed: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => QuanLy_TruyVet()));
                },
              ),
            )
          ],
